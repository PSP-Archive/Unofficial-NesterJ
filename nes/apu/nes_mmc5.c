...
{
	int32 accum = 0;
	accum += MMC5SoundSquareRender(&g_apu_t.mmc5.square[0]);
	accum += MMC5SoundSquareRender(&g_apu_t.mmc5.square[1]);
	return accum >> 8;
}
...
static void MMC5SoundVolume(uint32 volume)
{
	...
	g_apu_t.mmc5.square[0].mastervolume = volume;
	g_apu_t.mmc5.square[1].mastervolume = volume;
}
			...
			case 0x5004: case 0x5006: case 0x5007:
				{
					...
					int port = address & 3;
					g_apu_t.mmc5.square[ch].regs[port] = value;
					g_apu_t.mmc5.square[ch].update |= 1 << port;
				}
				break;
			case 0x5011:
				g_apu_t.mmc5.da.output = ((int32)value) - 0x80;
				break;
			case 0x5010:
				g_apu_t.mmc5.da.key = (value & 0x01);
				break;
			case 0x5015:
				if (value & 1)
					g_apu_t.mmc5.square[0].key = 1;
				else
				{
					g_apu_t.mmc5.square[0].key = 0;
					g_apu_t.mmc5.square[0].length = 0;
				}
				if (value & 2)
					g_apu_t.mmc5.square[1].key = 1;
				else
				{
					g_apu_t.mmc5.square[1].key = 0;
					g_apu_t.mmc5.square[1].length = 0;
				}
				break;
		}
	}
...
{
	MMC5SoundSquareReset(&g_apu_t.mmc5.square[0]);
	MMC5SoundSquareReset(&g_apu_t.mmc5.square[1]);
	MMC5SoundDaReset(&g_apu_t.mmc5.da);
