...
__inline static uint32 PGCALC(OPLL_OP *pop)
{
	if (pop->pg_vib)
	{
		...
		{
			pop->vib_cycles += g_apu_t.ym2413s.lfo[1].output;
...
__inline static uint32 EGCALC(OPLL_OP *pop)
{
	...
	if (pop->eg_am) egout += g_apu_t.ym2413s.lfo[0].output;
	switch (pop->eg_mode)
	{
		case EGM_A:
			{
...
__inline static int32 CHCALC(OPLL_CH *pch)
{
	...
	return LogToLinear(pch->output + g_apu_t.ym2413s.mastervolume, (LOG_LIN_BITS - 21));
}
...
static int32 OPLLSoundRender(void)
{
	...
	uint8 ch;
#if OPLL_CH_MAX == 9
	uint8 chmax = (g_apu_t.ym2413s.rhythmc & 0x20) ? 6 : 9;
#else
	const uint8 chmax = 6;	/* VRCVII */
#endif
	for (ch = 0; ch < 2; ch++) LFOCALC(&g_apu_t.ym2413s.lfo[ch]);
	for (ch = 0; ch < chmax; ch++)
	{
		accum += CHCALC(&g_apu_t.ym2413s.ch[ch]);
	}
	return accum >> 8;
}
...
{
	g_apu_t.ym2413s.mastervolume = (volume << (LOG_BITS - 8)) << 1;
}

static void OPLLSoundWriteAddr(uint32 address, uint8 value)
{
	g_apu_t.ym2413s.adr = value;
}

static void OPLLSoundWriteData(uint32 address, uint8 value)
{
	int ch = g_apu_t.ym2413s.adr & 0xF;
	switch (g_apu_t.ym2413s.adr & 0xF8)
	{
		case 0x00:
			g_apu_t.ym2413s.usertone[g_apu_t.ym2413s.adr] = value;
			g_apu_t.ym2413s.toneupdate = ~0;
			break;
#if OPLL_CH_MAX == 9
		case 0x08:
			if (ch != 0xE) break;
			g_apu_t.ym2413s.rhythmc = value;
			/* rhythmmode */
			break;
#endif
		case 0x10:
		case 0x18:
			...
			g_apu_t.ym2413s.ch[ch].freql = value;
			g_apu_t.ym2413s.ch[ch].update |= OPLL_UPDATE_FREQL;
			break;
		case 0x20:
		case 0x28:
			if (ch >= OPLL_CH_MAX) break;
			g_apu_t.ym2413s.ch[ch].freqh = value;
			g_apu_t.ym2413s.ch[ch].update |= OPLL_UPDATE_FREQH;
			break;
		case 0x30:
		case 0x38:
			if (ch >= OPLL_CH_MAX) break;
#if OPLL_CH_MAX == 9
			if ((g_apu_t.ym2413s.rhythmc & 0x20) && (ch > 5))
			{
				if (ch != 6)
					g_apu_t.ym2413s.ch[ch].op[0].tl = ((value >> 4) & 0xF) << 2;
				g_apu_t.ym2413s.ch[ch].op[1].tl = (value & 0xF) << 2;
			}
			else
#endif
			{
				if(g_apu_t.ym2413s.ch[ch].toneno != (value >> 4))
				{
					if (value >> 4)
					{
						g_apu_t.ym2413s.ch[ch].toneno = value >> 4;
						_memcpy(g_apu_t.ym2413s.ch[ch].tone, oplltone[value >> 4], 8);
						g_apu_t.ym2413s.ch[ch].update |= OPLL_UPDATE_TONE;
					}
					else if ((g_apu_t.ym2413s.ch[ch].toneno != 0x10) || (g_apu_t.ym2413s.toneupdate & (1 << ch)))
					{
						g_apu_t.ym2413s.toneupdate &= ~(1 << ch);
						g_apu_t.ym2413s.ch[ch].toneno = 0x10;
						_memcpy(g_apu_t.ym2413s.ch[ch].tone, g_apu_t.ym2413s.usertone, 8);
						g_apu_t.ym2413s.ch[ch].update |= OPLL_UPDATE_TONE;
					}
				}
				g_apu_t.ym2413s.ch[ch].op[1].tl = (value & 0xF) << 2;
...
//	double a;
	uint32 cps;
	_memset(&g_apu_t.ym2413s, 0, sizeof(OPLLSOUND));
	cps = DivFix(NES_BASECYCLES, 6 * 72 * SAMPLE_RATE, 18);
	for (i = 0; i < OPLL_CH_MAX; i++)
	{
		g_apu_t.ym2413s.ch[i].cps = cps;
		g_apu_t.ym2413s.ch[i].toneno = 0x10;
		g_apu_t.ym2413s.ch[i].fb = 31;
		g_apu_t.ym2413s.ch[i].output = 0 + (LOG_KEYOFF << 1);
		g_apu_t.ym2413s.ch[i].op[0].sintblp = sintbl;
		g_apu_t.ym2413s.ch[i].op[1].sintblp = sintbl;
	}
/*
	w = 1 << (PG_BITS - 2);
...
*/
	g_apu_t.ym2413s.lfo[0].cps = cps;
	g_apu_t.ym2413s.lfo[0].spd = DivFix(NES_BASECYCLES, (uint32)((6 * 72 * (1 << AM_BITS)) * 3.7), 18);
	g_apu_t.ym2413s.lfo[0].adrmask = (1 << AM_BITS) - 1;
	g_apu_t.ym2413s.lfo[0].table = amtbl;
	g_apu_t.ym2413s.lfo[0].output = g_apu_t.ym2413s.lfo[0].table[0];
	/* VIB */
/*
	w = 1 << PM_BITS;
	for (i = 0; i < w; i++)
...
*/
	g_apu_t.ym2413s.lfo[1].cps = cps;
	g_apu_t.ym2413s.lfo[1].spd = DivFix(NES_BASECYCLES, (uint32)((6 * 72 * (1 << PM_BITS)) * 6.4), 18);
	g_apu_t.ym2413s.lfo[1].adrmask = (1 << PM_BITS) - 1;
	g_apu_t.ym2413s.lfo[1].table = pmtbl;
	g_apu_t.ym2413s.lfo[1].output = g_apu_t.ym2413s.lfo[1].table[0];
}
