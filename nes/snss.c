...
int MemSaveSNSS(unsigned char **ssp, NES* nes){
	...
	if(snss==NULL)
		return 0;
	//	memset(snss, 0, 0x10+sizeof(SnssBaseBlock)+sizeof(SnssVramBlock)+sizeof(SnssSramBlock)+sizeof(SnssMapperBlock)+sizeof(SnssControllersBlock)+sizeof(SnssSoundBlock));
	memset(snss, 0, 0x20);
	p = snss+0x20;
